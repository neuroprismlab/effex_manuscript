...
plot_results <- function(estimate = 'd', fn_basedir, v_data, motion_method_t = "threshold", motion_method_not_t = "regression", save_plots = TRUE) {
...
summary_data <- get_study_summaries(v_data$data, v_data$study, motion_method_t, motion_method_not_t, meta = FALSE, estimate,ci_lb,ci_ub)
...
# get summaries
summary_data__meta <- get_study_summaries(v_data$meta_category$data, v_data$meta_category$study, motion_method_t, motion_method_not_t, meta = TRUE, estimate,ci_lb,ci_ub)
...
if (plot_non_mv) {
  ...
  res <- est_distribution_params(summary_data, summary_data__meta, "Point Estimates", paste0(fn_basedir,'point_est'))
  res_cons <- est_distribution_params(summary_data_cons, summary_data_cons__meta, "Conservative Estimates", paste0(fn_basedir,'cons_est'))
    
  # repeat, but for large N studies
  print('Fitting lines for large N studies only')
  res_large <- est_distribution_params(summary_data[summary_data$n > n_large_threshold,], summary_data__meta[summary_data__meta$n > n_large_threshold,],"Point Estimates (n > 900)", paste0(fn_basedir,'point_estimates_n900'))
}

# get multivariate effect sizes
print('Fitting lines for multivariate effect sizes')
res_mv <- est_distribution_params(summary_data, summary_data__meta, "Multivariate Effect Sizes", paste0(fn_basedir,'mv_est'), plot_type = "mv")
...
plot_required_n_panel(required_n_df, if_mv = FALSE, cat_colors,fn_basedir)

# Multivariate setup
required_n_df_mv <- make_required_n_df(cats, d, sigmas_master, res_mv = res_mv, mv = TRUE)
plot_required_n_panel(required_n_df_mv, if_mv = TRUE, cat_colors,fn_basedir)
...
get_study_summaries <- function(data, study, motion_method_t, motion_method_not_t, meta, estimate,ci_lb,ci_ub) {
...
est_distribution_params <- function(df, df_meta, main_title, fn, plot_type = "sd") {
...
plot_required_n_panel <- function(df, if_mv = FALSE, cat_colors,fn_basedir) {
